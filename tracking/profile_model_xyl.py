...
def evaluate_vit(model, template, search, text):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search, text, text),
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search, text, text)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search, text, text)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    if args.script == "ostrack":
        ...
        text = torch.ones(bs, t_len)
        text = text.long().to(device)  # RuntimeError: Expected tensor for argument #1 'indices' to have one of the following scalar types: Long, Int; but got torch.FloatTensor instead (while checking arguments for embedding)

        merge_layer = cfg.MODEL.BACKBONE.MERGE_LAYER
        if merge_layer <= 0:
            evaluate_vit(model, template, search, text)
        else:
            evaluate_vit_separate(model, template, search, text)
